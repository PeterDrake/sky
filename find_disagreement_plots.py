...
def make_plots(df, data_directory, save_path, i, num_rows):
	...
	for _, row in df.iterrows():  # Note: itertuples would be slightly faster, but iterrows is easier to use.
		...
		ax0.set_title("Sky Image", labelsize=28)
		ax0.set_xticks([])
		ax0.set_yticks([])
		ax1.imshow(tsi_mask)  # ax1 is the tsi decision image
		ax1.set_title("TSI Decision Image", labelsize=28)
		ax1.set_xticks([])
		ax1.set_yticks([])
		ax2.imshow(net_mask)  # ax2 is the network decision image
		ax2.set_title("Network Decision Image", labelsize=28)
		ax2.set_xticks([])
		ax2.set_yticks([])
...
# dubious_agree_df = dubious_agree_df.loc[dubious_agree_df['timestamp_utc'] == 20130701202830]
